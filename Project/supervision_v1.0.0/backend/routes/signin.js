const expressFunction = require('express');
const mongoose = require('mongoose');
const jwt = require('jsonwebtoken')
const bcrypt = require('bcrypt');
const router = expressFunction.Router()
const key = 'My_Key';

var Schema = require('mongoose').Schema
const userSchema = Schema({
    studentid: String,
    email: String,
    username: String,
    password: String,
    role: String,
    tell: String,
}, {
    collection: 'users'
});

let User
try {
    User = mongoose.model('users')
} catch (error) {
    User = mongoose.model('users', userSchema)
}

const compareHash = async (plaintest, hashText) => {
    return new Promise((resolve, reject) => {
        bcrypt.compare(plaintext, hashText, (err, data) => {
            if (err) {
                reject(new Error('Error bcrypt compare'))
            } else {
                resolve({ Status: dara })
            }
        })
    })
}

const findUser = (username) => {
    return new Promise((resolve, reject) => {
        User.findOne({ username: username }, (err, data) => {
            if (err) {
                reject(new Error('Cannot find username!'))
            } else {
                if (data) {
                    resolve({ id: data.id, email: data.email, username: data.username, password: data.password, roles: data.roles })
                } else {
                    reject(new Error('Cannot find Username !!!'))
                }
            }
        })
    })
}

router.route('./signin')
    .post(async (req, res) => {
        const playload = {
            username: req.body.email,
            password: req.body.password,
        }

        console.log(playload)
        
        try{
            const result = await findUser(playload.username)
            const loginStatus = await compareHash(playload.password, result.password)
            const status = loginStatus.state;
            console.log("Test", result)
            if(status){
                const token = jwt.sing(result, key, { expireseIn: 60*5 })
                res.status(200).json({ result, token, status})
            } else {
                res.status(200).json({ status })
            }
        } catch (error) {
            res.status(404).send(error)
        }
    })
module.exports = router